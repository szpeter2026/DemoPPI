...
## Storage 头像桶（头像上传需先配置）

1. 在 Supabase Dashboard → **Storage** 中创建 bucket：名称 `avatars`，设为 **Public**
2. 在 **SQL Editor** 中执行 `supabase/migrations/20260220000001_storage_avatars.sql` 中的 policy 部分（若通过 Dashboard 创建 bucket，可跳过 `insert into storage.buckets`）
...
- 若出现 "relation already exists"：说明表已存在，可跳过或先删除后重建
- 执行成功后，在 **Table Editor** 中应能看到 `profiles`、`connections`、`value_tags`、`interest_tags` 四张表
